...
def run_two_mode_benchmark(seed=42, save_report=True):
    ...
    b_unreliable = sum(1 for k in test_scans if "rot_" in k and results[k]["B"]["verdict"] == "UNRELIABLE_ALIGNMENT")
    ...
    escalations = [k for k in auto_defect_scans 
                   if results[k]["auto"].get("escalated", False)]
    ...
    fallback = bool("A" in str(jitter_auto.get("mode_used", "")) 
                    and not jitter_auto.get("escalated", False))
