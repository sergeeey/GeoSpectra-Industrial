...
class PatchAnomalyDetector:
    ...
    def detect(self, scan_points, use_patches=True):
        ...
        if use_patches and self.ref_patch_bank is not None:
            from core.registration import align_scan_to_reference
            ...
            try:
                ...
                if reg_report["confidence"] < self.registration_confidence_threshold:
                    return self._make_result(
                        ...
                        global_score=global_score, patch_score=0.0,
                ...
                return self._make_result(
                    verdict="UNRELIABLE_ALIGNMENT", confidence=0.0,
                    global_score=global_score, patch_score=0.0,
        ...
        if reg_report and reg_report.get("status") == "FAIL":
            verdict, confidence = "UNRELIABLE_ALIGNMENT", 0.5
        elif patch_score >= self.patch_threshold and not is_coherent:
            verdict, confidence = "LOW_COHERENCE", min(1.0, patch_score / 6.0)
        elif patch_score >= self.patch_threshold and is_coherent:
            verdict, confidence = "LOCAL_DEFECT", min(1.0, patch_score / 6.0 + 0.5)
        elif global_score >= self.global_anomalous_threshold:
            verdict, confidence = "GLOBAL_DEFORMED", min(1.0, global_score / 8.0 + 0.5)
        elif global_score >= self.global_deformed_threshold:
            verdict, confidence = "GLOBAL_DEFORMED", min(1.0, global_score / 4.0 + 0.5)
        elif global_score >= 0.5:
            verdict, confidence = "NOISY", min(1.0, global_score / 2.0)
        else:
            verdict, confidence = "NORMAL", min(1.0, 1.0 - global_score / 2.0)
        
        combined_score = max(global_score, patch_score)
        
        return {
            "verdict": verdict, "confidence": round(confidence, 3),
            "global_score": round(global_score, 3), "patch_score": round(patch_score, 3),
            "combined_score": round(combined_score, 3),
            "n_anomalous_patches": n_anomalous, "top_patches": top_patches,
            "heatmap": heatmap, "registration": reg_report,
            "thresholds": {
    ...
    def _run_patch_layer(self, aligned_scan):
        ...
        return {"patch_score": patch_score, "n_anomalous": n_anomalous,
                "top_patches": top_patches, "heatmap": heatmap}
    ...
    def _make_result(self, verdict, confidence, global_score, patch_score,
        ...
        result = {
            ...
            "n_anomalous_patches": 0, "top_patches": [], "heatmap": [],
    ...
    def load(cls, path):
        with open(path) as f:
            data = json.load(f)
        det = cls(
            n_patches=data["n_patches"], patch_size=data["patch_size"],
